...
4. Changes to the binary build or install scripts are proven locally before they reach a release: `npm run build:binary -w packages/cli` and `node packages/cli/scripts/smoke-binary.mjs packages/cli/release/mbd-<os>-<arch>/mbd --live` on your machine, the Linux targets inside `docker run --platform linux/<arch> node:24-bookworm`, and `scripts/install.sh` against a local server with `MBD_DOWNLOAD_URL`.
